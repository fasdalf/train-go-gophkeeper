...
func TestDBProxy_runMigrations(t *testing.T) {
	oldEnv := os.Getenv(testDBDSNENVKey)
	os.Setenv(testDBDSNENVKey, testDBDSNENVDefault)
	defer os.Setenv(testDBDSNENVKey, oldEnv)

	got, _ := NewTestDbStorage()
	defer got.Teardown(context.Background())
	err := got.runMigrations(context.Background(), []string{"DO NOT CREATE TABLE"})

	if err == nil {
		t.Errorf("runMigrations() did not set error")
	}
}
